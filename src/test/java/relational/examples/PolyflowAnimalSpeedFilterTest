...
class PolyflowAnimalSpeedFilterTest {
    ...
    void testTupleFiltering() {
        ...
        boolean lowAccepted = lowSpeedTuple.getValue2() > THRESHOLD;
        boolean highAccepted = highSpeedTuple.getValue2() > THRESHOLD;
    ...
    void testTupleFilteringBoundarySpeed() {
        Quartet<Long, String, Double, Long> boundaryTuple = new Quartet<>(3L, "gazelle", THRESHOLD, 0L);
        boolean accepted = boundaryTuple.getValue2() > THRESHOLD;
        assertFalse(accepted, "Tuple with speed = threshold should be rejected");
    }
    ...
    void testTupleFilteringZeroAndNegativeSpeed() {
        ...
        assertFalse(zeroSpeed.getValue2() > THRESHOLD, "Tuple with speed 0 should be rejected");
        assertFalse(negativeSpeed.getValue2() > THRESHOLD, "Tuple with negative speed should be rejected");
    }
    ...
    @Test
    void testAlternatingLowHighSpeeds() {
        List<Quartet<Long, String, Double, Long>> tuples = List.of(
                new Quartet<>(1L, "lion", 10.0, 0L),
                new Quartet<>(2L, "zebra", 16.0, 100L),
                new Quartet<>(3L, "gazelle", 14.0, 200L),
                new Quartet<>(4L, "dog", 18.0, 300L)
        );
        ...
        assertEquals(16.0, accepted.get(0).getValue2());
        assertEquals(18.0, accepted.get(1).getValue2());
    }
    ...
    void testAllBelowThreshold() {
        ...
        assertTrue(accepted.isEmpty(), "No tuples should pass threshold");
    }
    ...
    void testAllAboveThreshold() {
        ...
        assertEquals(2, accepted.size(), "All tuples should pass threshold");
    }

    @Test
    void testEdgeCasesAroundThreshold() {
        List<Quartet<Long, String, Double, Long>> tuples = List.of(
                new Quartet<>(1L, "lion", 14.9, 0L),
                new Quartet<>(2L, "zebra", 15.0, 0L),
                new Quartet<>(3L, "gazelle", 15.1, 0L)
        );
        ...
        assertEquals(15.1, accepted.get(0).getValue2());
    }
    ...
    void testLongSequence() {
        ...
        assertEquals(10, accepted.size(), "Half of the tuples should be accepted");
    }
}
